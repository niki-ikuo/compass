...
└── services/
    ...
    ├── ai-client.ts        # OpenAI 互換 API / SSE
    ├── settings.ts         # 設定の読み書き
...
| `ai:*` | チャット送信、チャンク / 完了 / エラーイベント、インライン補完 |
| `settings:*` | 設定の取得・保存 |
...
1. Renderer が現在ファイル・選択範囲・参照コンテキストを組み立てる（Ask / Edit）
2. Main が `.compass` の構造索引を確保し、AI 向けコンテキストに付与する
3. `ai:chat` で Main にリクエストを送る
4. Main の `ai-client` が OpenAI 互換 API へ SSE 接続する
5. トークンを `ai:chunk` で逐次 Renderer に返す
6. 完了時に `ai:done`、失敗時に `ai:error`
