...
**実装状態:** 実装済み（チャットヘッダ UI・アプリ／ワークスペース既定・用途別 system ロール・Agent の document/data 向け light verify・data 用途の `profileData` / `queryData`）
...
| **document** | 要約・構成案 | `.md` 等の修正提案 | 見出し単位の読取・整形、light verify（見出し / リンク） |
| **data** | スキーマ説明・品質指摘 | JSON / YAML / CSV / TSV の修正 | `profileData` / `queryData`、整形、スキーマ verify |
...
- `profileData` / `queryData`（data 用途のみ）: 列プロファイル + 読み取り専用インメモリ SQLite SELECT（`agent-data-sandbox.ts`）
- data verify 強化: 先頭列 / id キーの重複、列型の混在、TSV 対応
