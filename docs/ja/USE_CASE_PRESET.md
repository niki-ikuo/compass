...
Compass を「コード専用 IDE」から、ローカルフォルダを相手にする AI ワークスペースへ広げるための **用途プリセット**（`code` / `document` / `data` / `general`）の製品仕様です。
...
強み（フォルダ・Ask / Edit / Agent・差分承認・ターミナル）はそのまま使い、対象をテキスト中心の資料・メモ・データに広げる。

この方針では次をやらない。

- コードエディタ（Monaco）を捨てること
- 巨大なダッシュボード化
- 「なんでも屋アプリ」への全面転換
- Office 連携を前提にすること
...
| 軸 | 値 | 変えるもの |
|---|---|---|
| 用途 | `code` / `document` / `data` / `general` | システムプロンプトの役割・文体・優先ファイル・注意点 |
...
|---|---|---|---|
| `code` | コード | 実装・リファクタ・レビュー | コーディングアシスタント（現状相当） |
