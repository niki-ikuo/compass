...
- ワークスペース構造索引（`.compass/` — テキストは索引 / バイナリは除外。ファイル一覧、コード向け import グラフ等を AI コンテキストに付与）
...
- クリップ（Clip）— ホットキー取込 → inbox / outbox → 検品コピー。任意でトレイ常駐（閉じても取込維持）。詳細は [DESK_LOOP.md](./DESK_LOOP.md)
...
| Office / OpenDocument | OS 既定アプリで開く（Monaco エディタでは開かない）。エクスプローラーにも「既定のアプリで開く」あり。`.docx` は抽出テキストを索引・検索・@メンションに載せ、「Markdown に要約」で `.summary.md` サイドカーをプレビュー→適用 |
| PDF | アプリ内プレビュー。抽出テキストを索引・検索。「Markdown に要約」は `.docx` と同様 |
...
本質は「エディタ + 文脈を持った AI + 変更の適用」。構造索引・用途プリセット・Ask / Edit / Agent（v2.0 Phase 0–4）・インライン補完・ハイブリッド意味検索まで実装済み。MCP / プラグインは後続とする。
