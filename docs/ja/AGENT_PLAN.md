...
- ツール: `proposeActions({ actions: WorkspaceAction[] })` — 既存ファイルの修正は **`applyPatch`**（unified diff）を優先、`writeFile` は新規／短い全置換向け
...
- **適用失敗 → 再提案:** 適用エラー時はプレビューを残して再試行可能。Agent 承認待ちなら **Agentに修正させる** でプレビューを閉じ、失敗観測を返してループが再提案できる
- **検証ループ:** `verify` ツールでプロジェクトの test / lint / typecheck をスクリプト（または安全なフォールバック）経由で実行。適用後は fix → verify → fix をプロンプトで定石化
...
- **計画レイヤ:** `updateTodo` チェックリスト + `checkpoint` 再開要約。Continue 時およびフォローアップの事前コンテキストに再注入（`electron/services/agent-plan.ts`）
- **コンテキスト保持（RAG 前段階）:** 観測の自動要約 + `remember` ツールによる作業メモリ（`agent-memory.ts`）；エントリポイント / exports / シンボルを強調した `.compass` 要約（`project-indexer.ts`）；ラン内 `readFile` スマートキャッシュと `force=true` バイパス（`agent-read-cache.ts`）

**完了条件:** Compass が既に支える主要プロバイダで日常利用に耐える。

**後回し:** tools 非対応時の自動 Ask/Edit フォールバック、プロバイダ別の Agent トグル非表示。
