...
**v2.0 Agent** 自律実行（ツールループ・コマンド・複数ステップ）の段階的実装記録です。**Phase 0–4 は現行製品に出荷済み**（`package.json` の version 2.2.x）。今日のランタイムの動きは [AGENT.md](./AGENT.md) を参照。関連: [SPEC.md](./SPEC.md) §10、[ARCHITECTURE.md](./ARCHITECTURE.md)。

フェーズチェックリスト・決定済みポリシー・後続ロードマップを残す文書であり、「未着手の計画書」ではない。
...
5. **OpenAI 互換 `tools` API** — native tool calling（`compass-actions` テキストプロトコルではない）。tools 非対応プロバイダでは Agent トグルを隠し、必要なら Edit への誘導フォールバックを出す。
...
| tools 非対応プロバイダ | Agent 非表示 / 警告 / フォールバック | **非対応時は Agent を非表示**；万一選ばれていた場合は **Edit 誘導フォールバック**；それ以外は明確なエラー（`ai.agentToolsUnsupported`） |
