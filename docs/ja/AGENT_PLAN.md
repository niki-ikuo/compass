...
- ツール: `writeFile` / `mkdir` / `deleteFile` / `deleteDir`（または一括の `proposeActions`）。
- 破壊的・書き込みツールはループを **一時停止** し、プレビューを開く（可能な範囲で Edit UI を再利用）。
- 適用または却下後、続行するか停止するかはプロダクト方針としてドキュメント化する。
...
- 制御付き `exec`: cwd = ワークスペース、timeout、stdout/stderr 上限、デフォルトは非対話。
- 危険パターンは deny list、または都度承認。
- ユーザーの対話 PTY を Agent ループから直接操作しない。
...
- 失敗後の部分適用 / 再試行
- 長時間ランの進捗とキャンセル意味の明確化
- `tools` 非対応プロバイダ向けフォールバック（Agent 非表示、警告、Edit 相当への劣化など）
- 履歴再生時に tool ステップが壊れないこと
- ガードレール: ターン上限、ペイロードサイズ上限、ツールログからの秘密情報のマスキング
...
| トピック | 選択肢 / メモ | 決定 |
|----------|---------------|------|
| 却下後の承認ポリシー | ラン停止 vs 書き込みなしで続行 | _未定_（Phase 2） |
| 書き込みツールの粒度 | 一括 `proposeActions` vs 複数ツール | _未定_（Phase 2） |
| tools 非対応プロバイダ | Agent 非表示 / 警告 / フォールバック | _未定_（Phase 4） |
| ステップ永続化の形 | メッセージ上 vs 別ラン記録 | **`ChatMessage.agentSteps`** |
| exec の許可方式 | まず allow-list vs deny-list | _未定_（Phase 3） |
