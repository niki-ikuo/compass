# 仕事机ループ（Desk Loop）— S級5機能 仕様

[English](../DESK_LOOP.md) | **日本語**

状態: **仕様（未実装）**。関連: [SPEC.md](./SPEC.md)、[ARCHITECTURE.md](./ARCHITECTURE.md)、[TEXT_WORKSPACE_PLAN.md](./TEXT_WORKSPACE_PLAN.md)、[AI_APPLY_UNDO.md](./AI_APPLY_UNDO.md)、[USE_CASE_PRESET.md](./USE_CASE_PRESET.md)。
...
| 問い | 答え |
|------|------|
| 何を作るか？ | 取込ホットキー / 仕事机 / 下書き工場 / 検品ゲート / 週次ダイジェスト |
...
[クリップボード] ＋ status: ready（任意更新）
        ↓ 随時
[仕事机] inbox 処理済み / outbox 一覧 / ダイジェスト生成
        ↓
...
5. 仕事机で inbox を処理済みにでき、outbox が見える  
6. ダイジェスト1枚が生成できる  
...
初回: ワークスペースオープン時、または仕事机／取込／下書き／ダイジェストのいずれかの初回利用時に不足ディレクトリを作成する。
...
| 仕事机の表示件数など | `.compass/desk/settings.json`（任意。無くても既定値） |
...
2. 既定ショートカット: `Ctrl+Shift+Alt+V`（Windows）。設定画面で変更・無効化できる。  
3. 発火時:
...
```
   ...
   7. ウィンドウを前面化（focus）し、作成ファイルをエディタで開く。設定で「仕事机を開く」も選択可（既定はファイルを開く）。  
...
- **設定:** 「仕事机」セクション — ショートカット録音 UI、有効/無効、着地後の動作（ファイルを開く / 仕事机を開く）  
...
1. 左サイドバーに **仕事机** タブ（またはビュー）を追加する。アイコンは既存トーンに合わせ、ダッシュボード化しすぎない。  
2. 画面構成は **3セクションのみ**（初期）:
...
- **リスト＋短いメタ** の一枚構成。  
- ブランド／製品名を仕事机ヒーローで押し潰さない（既存シェル内ビュー）。
...
| `archived` | 仕事机の既定一覧から隠す（ファイルは残す） |

### 6.5 起動 UI

1. コマンドパレット / チャットヘッダ / 仕事机: **下書きを作る…**  
2. プリセット選択（4択）  
3. 文脈:
...
- Outbox ファイルがアクティブ、または仕事机 Outbox 行の **検品してコピー**  
...
- アクション:
  - **本文をコピー** — findings が error 0 件ならそのまま。warning のみなら確認なしで可  
  - **それでもコピー** — error が残っていても確認ダイアログ後にコピー  
  - **閉じる**
...
1. 起動: 仕事机の **ダイジェストを作る**、またはコマンドパレット。  
...
| `helps/en/...` | 英語版 |
| [SPEC.md](./SPEC.md) | 機能一覧に仕事机ループを1行追加 |
