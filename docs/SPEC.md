...
| API Key | ローカル暗号化保存（Windows Credential Manager） |
| モデル | `gpt-4o-mini` |
| 温度 | 0.2 |
| Max Tokens | 4096 |
...
```
現状（ターミナル / .compass 構造索引 / Ask・Edit 提案モード まで実装）
 └─ v1.x: インライン補完（Tab補完）
     └─ v2.0: Agent 自律実行（ツールループ・コマンド実行・複数ステップ）
         └─ v2.1: コードベース意味検索（RAG / 埋め込み）
             └─ v3.0: MCP、マルチモデル、プラグイン
```
...
Cursorの本質は「エディタ + 文脈を持ったAI + 変更の適用」。現状は構造索引と Edit（提案適用）まで実装済み。自律実行ループ・RAG・インライン補完は後続とする。
