---
title: Compass Help
keywords:
  - ヘルプ
  - 目次
  - はじめに
  - FAQ
category: index
---

# Compass Help

困ったとき・やりたいことが決まっているときに読むページです。機能メニューではなく、目的から探してください。

## 初めて使う

- [はじめに](getting-started/welcome.md)
- [インストール](getting-started/install.md)
- [AIプロバイダー設定](getting-started/ai-provider.md)
- [プロジェクトを開く](getting-started/open-project.md)
- [ワークスペースを検索](getting-started/search.md)

---

## やりたいこと

- [AIチャットを使う](ai/chat.md)
- [Agentを使う](ai/agent.md)
- [ファイルを検索する](getting-started/search.md)
- [GitHubを使う](integrations/github.md)（未対応）
- [MCPを使う](integrations/mcp.md)（未対応）

---

## 困った

- [よくあるエラー](troubleshooting/common-errors.md)
- [FAQ](troubleshooting/faq.md)

---

## このヘルプについて

- 1ページは短く保ちます（だいたい300〜500行以内）
- 関連ページへ相互リンクします
- 未実装の機能は「未対応」と明記します（偽の手順は書きません）
- API（または Ollama）を設定すると、メニューの **AIヘルプ** でヘルプ記事を根拠に聞けます
