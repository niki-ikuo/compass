...
- 適用後は直近の適用を取り消せる（Edit と同じ。メッセージ横・適用履歴・このチャット分も可）
...
- CSV 等は応答中だけの一時 SQLite に取込まれる（永続 DB ではない）。ツール行の summary が `imported …`（新規取込）または `cached …`（同一ラン内の再利用）なら取込済み
...
**Q. CSV を Agent で調べたい**  
A. 用途を **データ** にして依頼してください（例:「sales.csv をプロファイルして欠損率をまとめて」）。列の調査や読み取り専用クエリは、ファイル変更の承認前に行えます。一時 SQLite への取込成否は、ステップ表示の `imported` / `cached` / `import failed` で確認できます。
