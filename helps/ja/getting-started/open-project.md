...
| `rules.md` | ワークスペースルール（Ask / Edit / Agent に自動添付） |
| `glossary.md` | 用語集（任意。文書 verify と AI コンテキスト） |

索引や履歴は通常触る必要はありません。トーンや用語は **ファイル → ワークスペースルールを編集**（またはチャットのルールボタン）で `rules.md` を書けます。
...
A. 想定どおりです。編集は OS 既定アプリです。読む・探す・要約は Compass 側でできます（抽出テキストを索引・検索・@メンションに載せます。エクスプローラーの **Markdown に要約** で `.summary.md` を作れます）。

**Q. PDF はどう扱われる？**  
A. アプリ内でプレビューできます。テキストは索引・検索に入ります。ビューアやエクスプローラーから **Markdown に要約** / **既定のアプリで開く** も使えます。
