...
export const en: Record<MessageKey, string> = {
  ...
  'ai.agentModeReminder':
    '[Agent mode] Inspect with readFile / listDir / search / searchMeaning (prefer searchMeaning for topic/meaning); propose with proposeActions (actions must be an array; prefer applyPatch with unified-diff @@ hunks only—never *** Begin Patch—one patch per file; writeFile for new/tiny files); after apply, verify (test/lint/typecheck) then re-propose on failure; if verify only skips for missing scripts, omit that from the final reply. Prefer verify over exec for those checks (exec is short-lived, inside workspace; wipe commands blocked; writes need approval). For multiple asks or longer tasks, split with updateTodo first and do not finish while todos remain pending (also use checkpoint + remember). Batch related edits; split unrelated/large proposeActions. Re-reads may be cached (force=true to reload). Paths are relative; root is ".".',
  'ai.responseLanguage':
