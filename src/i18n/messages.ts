export const ja = {
  ...
  'ai.agentModeReminder':
    '[Agentモード] readFile / listDir / search で調査、変更は proposeActions（actions は配列。既存ファイルは applyPatch＝unified diff の @@ hunkのみ、*** Begin Patch 禁止、同一ファイルは1パッチ。新規・短い全置換は writeFile）。適用後は verify（test/lint/typecheck）→失敗なら再提案のループ。スクリプト無しのスキップは最終回答で触れない。verify 不可時のみ exec（短命・ワークスペース内。破壊的コマンドは拒否／書き込み系は承認）。長めのタスクは updateTodo / checkpoint / remember。同じファイルの再読はキャッシュされる（force=true で再取得）。パスは相対、ルートは "."。',
...
export const en: Record<MessageKey, string> = {
  ...
  'ai.agentModeReminder':
    '[Agent mode] Inspect with readFile / listDir / search; propose with proposeActions (actions must be an array; prefer applyPatch with unified-diff @@ hunks only—never *** Begin Patch—one patch per file; writeFile for new/tiny files); after apply, verify (test/lint/typecheck) then re-propose on failure; if verify only skips for missing scripts, omit that from the final reply. Prefer verify over exec for those checks (exec is short-lived, inside workspace; wipe commands blocked; writes need approval). For longer tasks use updateTodo + checkpoint + remember. Re-reads may be cached (force=true to reload). Paths are relative; root is ".".',
