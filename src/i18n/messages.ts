export const ja = {
  ...
  'ai.agentModeReminder':
    '[Agentモード] readFile / listDir / search / searchMeaning で調査（意味・トピックは searchMeaning）、変更は必ず proposeActions ツール（チャットや compass-actions では不可。actions は配列。既存ファイルは applyPatch＝unified diff の @@ hunkのみ、*** Begin Patch 禁止、同一ファイルは1パッチ。新規・短い全置換は writeFile。途中切れは1ファイルずつ再提案）。適用後は verify（test/lint/typecheck）→失敗なら再提案のループ。スクリプト無しのスキップは最終回答で触れない。verify 不可時のみ exec（短命・ワークスペース内。破壊的コマンドは拒否／書き込み系は承認）。複数依頼や長めのタスクは先に updateTodo で分解し、pending が残る間は終了しない（checkpoint / remember も活用）。関連変更は1提案、無関係・大規模は proposeActions を分割。同じファイルの再読はキャッシュされる（force=true で再取得）。パスは相対、ルートは "."。',
  'ai.responseLanguage':
