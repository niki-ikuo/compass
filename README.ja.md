...
![Compass — AI ワークスペース（Edit モードの差分プレビュー）](docs/assets/screenshot.png)

Windows 向けの AI ワークスペースです。ローカルフォルダを開き、メモ・文書・データ・コードなどテキスト全般を AI と一緒に書く・直す・整理できます。
...
- Monaco Editor によるテキスト編集（シンタックスハイライト。コード専用ではない）
- ワークスペース単位のファイルツリー
- AI チャット（ストリーミング）— **Ask** / **Edit** / **Agent**、用途プリセット（一般 / 文書 / データ / コード）
- インライン補完（ゴーストテキスト / Tab で確定）
- ワークスペース構造索引（`.compass/`）を AI コンテキストに利用
- 文書テンプレート（`.compass/templates/` の Markdown 雛形）
- AI 提案の差分プレビューと適用
- 統合ターミナル（xterm.js）
...
メモ・文書・データを一つのワークスペースで開き、AI に概要を聞いて整理する。

![一般用途 — ワークスペース概要と AI チャット](docs/assets/use-case-general.png)

### 文書

フォルダを指定して、プロジェクトからマニュアルやドキュメントを生成する。

![文書用途 — AI によるゲームマニュアル生成](docs/assets/use-case-document.png)

### データ

CSV を自然言語で質問し、表や集計結果をその場で得る。

![データ用途 — 自然言語による CSV 分析](docs/assets/use-case-data.png)

### コード

アプリやゲームを指示するだけでファイルを生成し、プレビューしながら進める。
